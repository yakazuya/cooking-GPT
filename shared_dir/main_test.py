...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            # img_dict,text_list = predict(file_path,num)
            all_dict = predict(file_path,num)
            # return predict(file_path,num)
            return process(all_dict)
...
def process(all_dict):
    return render_template('result.html',data=all_dict)
...
if __name__ == '__main__':
    ...
    # img_dict,text_list = predict(filename=None,num=None)
